...
// ===== 부서별 실행 횟수 (각 바로가기의 /api/hits 합산) =====
(function renderUsageStats() {
  const section = document.querySelector("#usageStats");
  const list = document.querySelector("#usageStatsList");
  const totalEl = document.querySelector("#usageStatsTotal");
  if (!section || !list || !totalEl) return;

  const requests = allDepartments.flatMap((department) =>
    department.links.map((link) => {
      let endpoint;
      try {
        endpoint = new URL("/api/hits", link.href).toString();
      } catch {
        return null;
      }
      return fetch(endpoint)
        .then((res) => res.json())
        .then((data) => ({ department: department.title, count: typeof data?.count === "number" ? data.count : 0 }))
        .catch(() => ({ department: department.title, count: 0 }));
    })
  ).filter(Boolean);
  ...
  Promise.all(requests).then((results) => {
    ...
    let grandTotal = 0;
    results.forEach(({ department, count }) => {
      totals.set(department, (totals.get(department) || 0) + count);
      grandTotal += count;
    });

    const departmentTitles = allDepartments
      .map((department) => department.title)
      .filter((title, index, arr) => arr.indexOf(title) === index);

    list.innerHTML = departmentTitles
      .map(
        (title) => `
          <div class="usage-stat-row">
            <span class="usage-stat-name">${title}</span>
            <span class="usage-stat-count">${(totals.get(title) || 0).toLocaleString("ko-KR")}</span>
          </div>
        `
      )
      .join("");

    totalEl.textContent = `총 ${grandTotal.toLocaleString("ko-KR")}회`;
